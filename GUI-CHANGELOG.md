...
> 本文件记录 Deepseek-GUI 每次开发变更（对内维护用）。项目介绍见 [README.md](./README.md)。

> **注意**：在 DeekSeel-TUI-GUI 等 TUI+GUI 混合工作区中，仓库**根目录**的 CHANGELOG.md 为 CodeWhale TUI **英文**发版日志（Cursor 点击 ./CHANGELOG.md 常会误跳过去）；**本文件**（GUI-CHANGELOG.md）才是 GUI **中文**开发记录。
...
### 120
* 更新时间：2026-06-20 14:30:00
* 涉及模块：GitHub 仓库 / CI / build-release 脚本
* 功能变更：GUI 独立上传至 ictorhuang868/Deepseek-GUI 仓库，与 TUI 分仓；关闭混入 CodeWhale 的 PR。
* 实现说明：独立仓库根目录即 GUI 工程；Actions 额外 checkout CodeWhale 仅用于编译 deepseek-tui sidecar；本地打包通过 CODEWHALE_ROOT 指向 TUI 克隆目录。
### 121
* 更新时间：2026-06-20 15:00:00
* 涉及模块：PROJECT.md / README.md
* 功能变更：新增独立项目介绍文件 PROJECT.md；README.md 恢复为开发变更记录专用。
* 实现说明：对外展示用 PROJECT.md（架构、快速开始、CI 打包）；README.md 保留 001 起更新记录，二者分离。
### 122
* 更新时间：2026-06-20 15:30:00
* 涉及模块：README.md / CHANGELOG.md / PROJECT.md
* 功能变更：项目介绍合并至 README 首页；开发变更记录迁移至 CHANGELOG.md（GitHub 首页展示介绍）。
* 实现说明：README 采用 PROJECT 内容；001–121 条目转移 CHANGELOG；PROJECT.md 改为指向 README 的短链接。
### 123
* 更新时间：2026-06-20 16:00:00
* 涉及模块：README.md / CHANGELOG.md
* 功能变更：补充文档说明，区分 GUI 中文 CHANGELOG 与仓库根目录 TUI 英文 CHANGELOG。
* 实现说明：README 新增「文档说明」表格；CHANGELOG 顶部增加混合工作区路径提示，避免误开根目录英文发版日志。
### 124
* 更新时间：2026-06-20 16:15:00
* 涉及模块：GUI-CHANGELOG.md / README.md / PROJECT.md
* 功能变更：GUI 开发记录由 CHANGELOG.md 重命名为 GUI-CHANGELOG.md，避免混合工作区链接误跳根目录英文 TUI 日志。
* 实现说明：Cursor/VS Code 多根 CHANGELOG 同名时相对链接解析到仓库根；新文件名在 TUI+GUI 并存目录下可唯一定位。
### 125
* 更新时间：2026-06-20 16:30:00
* 涉及模块：CodeWhale v0.8.62 / main.rs / build-release / tauri conf / CI
* 功能变更：GUI sidecar 升级至 codewhale-tui（v0.8.62）；保留 deepseek-tui 回退解析。
* 实现说明：克隆 CodeWhale@v0.8.62；打包与 CI 改用 codewhale-tui；main.rs 同目录与 PATH 双名查找；旧 DeekSeel-TUI-GUI 树仍可通过脚本自动识别 deepseek-tui。
### 126
* 更新时间：2026-06-20 17:00:00
* 涉及模块：dev-workflow.ps1 / gui-dev-workflow.mdc / README
* 功能变更：确立标准开发流程 CodeWhale → DeekSeel-TUI-GUI → Deepseek-GUI-git → GitHub。
* 实现说明：新增 dev-workflow.ps1（update-codewhale/sync-tui/sync-gui-git/push-gui）；Cursor 规则 gui-dev-workflow.mdc 强制顺序；sync-tui 不覆盖 Deepseek-GUI/。
### 127
* 更新时间：2026-06-20 14:03:33
* 涉及模块：dev-workflow.ps1 / compare-gap.py / gui-dev-workflow.mdc / README
* 功能变更：重制定开发流程——更新 CodeWhale 后先 compare-gap，开发完毕 sync-gui-git、push，最后 cleanup-workspace。
* 实现说明：新增 compare-gap.py 生成 docs/TUI-GUI-GAP.md；dev-workflow 增加 compare-gap/cleanup-workspace/full-cycle；修复 update-codewhale 对 origin/main 的 checkout。
### 128
* 更新时间：2026-06-20 14:06:32
* 涉及模块：compare-gap.py / dev-workflow.ps1 / .gitignore / TUI-GUI-GAP.md
* 功能变更：修复 compare-gap API 提取；gitignore 排除 sidecar 二进制与 .bak；从 GitHub 移除误提交的 .bak 与 exe。
* 实现说明：compare-gap 报告 API 不匹配 4 条（jobs/subagents/rlm/approvals 路径级）；robocopy 双斜杠排除 bin。
### 129
* 更新时间：2026-06-20 14:09:19
* 涉及模块：prune-tui-duplicates / dev-workflow / gui-dev-workflow.mdc / README
* 功能变更：工作区根目录删除与 CodeWhale 重复的 TUI 副本（46 项）；仅保留 Deepseek-GUI/.cursor。
* 实现说明：sync-tui 废弃；新增 prune-tui-duplicates.py；compare-gap 仅读 E:\\Coding\\CodeWhale；根目录 README.workspace.md 说明三目录分工。
### 130
* 更新时间：2026-06-20 14:11:21
* 涉及模块：工作区结构 / dev-workflow / README.workspace.md
* 功能变更：CodeWhale 与 Deepseek-GUI-git 迁入 DeekSeel-TUI-GUI 子目录；路径改为相对工作区自动推断。
* 实现说明：CodeWhale 自 E:\\Coding 移入；Deepseek-GUI-git 本地 clone；dev-workflow 默认 ../CodeWhale 与 ../Deepseek-GUI-git。
### 131
* 更新时间：2026-06-20 14:19:18
* 涉及模块：client.ts / agentRunsAlign / UserInputDialog / useConversation / JobsPanel / SubagentsPanel / RlmPanel / SnapshotsModal
* 功能变更：P0 API 对齐 v0.8.62——快照改 /v1/snapshots；Subagent 回退 /v1/agent-runs；Jobs/RLM 无 API 时友好提示；新增 request_user_input SSE 弹窗。
* 实现说明：submitUserInput 对接 POST /v1/user-input/{thread}/{id}；/undo /restore 随快照路径修复可用。
### 132
* 更新时间：2026-06-20 15:42:00
* 涉及模块：slashCommands / executeSlashCommand / ContextModal / client.ts / App.tsx / CodeView
* 功能变更：P1 斜杠命令补齐——/context、/skills、/skill、/new、/init、/purge、/goal、/status、/share、/lsp；/undo 优先 POST /v1/threads/{id}/undo。
* 实现说明：ContextModal 展示回合/条目/Token 分布；/context report|json|summary 文本导出；/lsp on|off 写入 localStorage 并由 CodeView 读取；/init /purge /goal /share 委托 Agent 执行（对齐 TUI 语义）。
### 133
* 更新时间：2026-06-20 16:28:00
* 涉及模块：slashCommands / executeSlashCommand / HomeModal / LinksModal / FeedbackModal / ChangelogModal / ThemeModal / StatuslineModal / tauri(main.rs) / guiPrefs / theme / App 状态栏
* 功能变更：P2+P3 斜杠命令全补齐（22 条 TUI 独有 → 0）；新增 /edit /system /home /links /feedback /change /exit /logout /hf /plugins /sidebar /theme /statusline /balance /cache /translate /verbose /voice* /doctor /slop /swarm。
* 实现说明：Tauri 新增 open_external_url、quit_app、clear_api_key、run_doctor、list_plugins、read_codewhale_file；四套 CSS 主题；/statusline 可配置底部芯片；/verbose 控制推理块默认展开；voice/cache/balance 为占位或说明性实现。
### 134
* 更新时间：2026-06-20 17:05:00
* 涉及模块：client.ts / optionalHttp.ts / compare-gap.py / OnboardingModal / DoctorPanel / Composer / CodeView / main.rs
* 功能变更：M4 启动——API 对齐（agent-runs 主路径、optionalHttp 分离、gap 0 不匹配）；首次启动 Onboarding；设置页 doctor；Composer/CodeView 外部编辑器；保存 LSP 提示。
* 实现说明：Subagent 改 GET /v1/agent-runs；jobs/rlm 迁至 optionalHttp.ts；Tauri spawn_external_editor + open_path_in_external_editor；无 API Key 时自动弹出 onboarding。
### 135
* 更新时间：2026-06-20 18:30:00
* 涉及模块：main.rs / voiceInput.ts / Composer / ExecPanel / PrPrefillModal / tabCompletion* / TasksView / slashCommands / executeSlashCommand
* 功能变更：M4 续完成——语音 ASR（Web Speech + Tauri transcribe_audio）；Composer 麦克风与 voice-send/control；Tab FIM /beta 开关；Tasks 页 CLI Exec；/pr PR 描述预填；斜杠 /exec /pr。
* 实现说明：Tauri 新增 transcribe_audio、run_cli_exec、generate_pr_prefill；tab_complete 支持 use_fim 走 /beta/completions；voice 失败时 MediaRecorder→WAV→ASR 回退。
### 136
* 更新时间：2026-06-20 19:15:00
* 涉及模块：client.ts / types.ts / QueueBar / queueStorage / App.tsx / executeSlashCommand / ContextModal / AgentHistoryPanel / styles.css
* 功能变更：M5——Queue/Stash 打磨（按线程持久化、排序、立即发送、单条暂存/弹回、内联编辑）；/undo 对齐 patch-undo 主路径；/save 对接 PUT /v1/sessions；ContextModal 回合调试时间线；历史面板「保存当前」。
* 实现说明：client 新增 patchUndoThread、saveSession、retryThread；undo 失败链 patch-undo → undo → snapshot；queue 键 ds_queue_{threadId}。
### 137
* 更新时间：2026-06-20 20:05:00
* 涉及模块：useComposerVim / Composer / FleetPanel / skill_install.rs / SkillsView / SettingsView / slashCommands / executeSlashCommand / client.ts / styles.css
* 功能变更：M6——Composer Vim 模式（Normal/Insert、/vim、设置开关）；Fleet 面板（/v1/fleet/runs、Worker 中断/重启）；GitHub 技能 install/uninstall（Tauri + SkillsView + /skill）。
* 实现说明：skill_install 下载 main/master tarball 解压至 ~/.codewhale/skills；卸载需 .installed-from 标记；Fleet 4 秒轮询；Vim Esc→Normal、Enter 在 Normal 不发送。
### 138
* 更新时间：2026-06-20 20:45:00
* 涉及模块：executeSlashCommand / App.tsx / FleetPanel / client.ts / compare-gap.py / SettingsView / GUI-TUI-ROADMAP
* 功能变更：M7 收尾——/terminal 切换底部 PTY 面板；/trust list|add|remove 斜杠内联；Fleet Worker GET 详情；compare-gap 前缀路由匹配修复（API 0 不匹配）。
* 实现说明：移除 SettingsTab terminal 死链；getFleetWorker 对接 /v1/fleet/workers/{id}；/trust add 无路径时 pickFolder。
### 139
* 更新时间：2026-06-20 15:12:00
* 涉及模块：发布打包 / build-release.ps1 / dist / src-tauri bundle
* 功能变更：M1–M7 全量重新打包 Windows 安装器（含 Vim、Fleet、Queue、Skill install 等近期 GUI 改动）。
* 实现说明：`npm run build` + `npm run tauri:build`；sidecar 复用 CodeWhale v0.8.62 `codewhale-tui.exe`（本地 release 链接 OOM，沿用 13:46 已编译 sidecar）；产物 0.1.0_x64：NSIS 22.07MB、MSI 32.59MB、gui 23.41MB、tui 68.75MB；路径 `src-tauri/target/release/bundle/`。已 push 触发 GitHub Actions 云端完整 sidecar+安装包构建。
### 140
* 更新时间：2026-06-20 16:20:00
* 涉及模块：translation.ts / translate.rs / useConversation / MessageItem / tauri.ts / SettingsView / executeSlashCommand
* 功能变更：/translate on 时思考块自动译为简体中文——流式阶段显示占位，完成后 Tauri 调用 chat/completions 翻译；设置 → 聊天可开关。
* 实现说明：needsTranslation 启发式对齐 TUI；仅桌面版 + API Key；失败回退原文并提示。
### 141
* 更新时间：2026-06-20 17:05:00
* 涉及模块：Composer / App(thread-prompt) / QueueBar / styles.css
* 功能变更：关闭 Composer、系统提示词、队列编辑框的 WebView2 拼写检查，消除输入区多行红色横线（误拼写下划线）。
* 实现说明：textarea/input 增加 spellCheck={false}、autoCorrect/autoCapitalize off；Markdown hr 分隔线改为更淡的 border-soft。
### 142
* 更新时间：2026-06-20 17:35:00
* 涉及模块：CodeView / MarkdownViewToggle / markdownPath.ts / styles.css
* 功能变更：README 及 .md/.mdc 文件支持 Cursor 风格 Preview / Markdown 切换；默认 Preview 渲染；Ctrl+Shift+V 快捷键。
* 实现说明：Preview 复用 Markdown 组件；Markdown 模式为 CodeMirror 源码编辑；视图偏好写入 localStorage（ds_md_editor_view）。
### 143
* 更新时间：2026-06-20 18:00:00
* 涉及模块：README.md / README.workspace.md
* 功能变更：全面更新项目总说明——M1–M7 能力、Markdown Preview、思考翻译、快捷键、文档索引；工作区 README 补充文档表。
* 实现说明：README 新增「更新记录」001（说明文档层变更）；功能细节仍以 GUI-CHANGELOG 为准。
### 144
* 更新时间：2026-06-20 21:30:00
* 涉及模块：发布打包 / build-release.ps1 / src-tauri bundle
* 功能变更：ed744e8 后全量重打包 Windows 安装器（README 更新、Markdown Preview、拼写检查修复、思考块翻译）。
* 实现说明：sidecar codewhale-tui 增量编译；NSIS 22.07MB、MSI 32.59MB、gui 23.4MB、sidecar 68.75MB；产物路径 `src-tauri/target/release/bundle/`。GitHub Actions 同步构建。
### 145
* 更新时间：2026-06-20 22:00:00
* 涉及模块：EditorEmptyState / styles.css
* 功能变更：编辑器空状态居中图标改为叠放文档线条，去掉原「圆点+叶片」造型。
* 实现说明：对齐 Cursor 欢迎页气质；避免顶部圆点被误读为突兀居中点。
### 146
* 更新时间：2026-06-20 22:15:00
* 涉及模块：工作区说明 / 本地目录
* 功能变更：废弃并删除 `E:\Coding\Deepseek-GUI-git` 旧克隆；统一仅使用 `DeekSeel-TUI-GUI\Deepseek-GUI-git` 对接 GitHub。
* 实现说明：`dev-workflow.ps1 publish` 默认即工作区内路径；README.workspace 补充勿另建重复 git 目录说明。
### 147
* 更新时间：2026-06-20 16:30:00
* 涉及模块：styles.css / useResizablePanels
* 功能变更：资源管理器默认加宽至 300px；文件树行高/字号/图标略增大；编辑器空状态提示与快捷键列表居中。
* 实现说明：旧 localStorage 侧栏 260px 自动升为 300；树行 26px、字号 14px；空状态 shortcuts 块 fit-content 居中。
### 148
* 更新时间：2026-06-20 16:45:00
* 涉及模块：useResizablePanels / styles.css（IDE grid）
* 功能变更：分栏拖拽下限——资源管理器最小 240px、编辑器最小 320px、Chat 最小 380px；拖到极限不可再压没组件。
* 实现说明：修复 grid 侧栏列 `minmax(0, …)` 导致窗口窄时列宽被压至 0；JS clamp 与 CSS `--sidebar-min` 等变量对齐。
### 149
* 更新时间：2026-06-20 20:15:00
* 涉及模块：main.rs / config_bridge.rs / useConversation.ts
* 功能变更：修复 GUI 保存 API Key 写入 `~/.deepseek` 而后端读 `~/.codewhale` 导致鉴权失败；Chat 现可显示 API/回合错误。
* 实现说明：`config_path` 与 sidecar 对齐（codewhale 优先）；`item.failed` 无前置 started 时也渲染；`turn.completed` 失败时顶部通知。
### 150
* 更新时间：2026-06-20 20:45:00
* 涉及模块：useConversation.ts / main.rs
* 功能变更：首次打开不再刷屏「回合失败」toast；冷启动从 REST 灌入历史并 SSE 续传；启动前自动同步 GUI 档案 Key 到 sidecar config。
* 实现说明：移除 turn.completed 弹 toast；GET /threads/{id} 后再订阅 events；`sync_active_profile_api_key_on_startup` 在 spawn 前执行。
### 151
* 更新时间：2026-06-20 21:10:00
* 涉及模块：Composer.tsx / styles.css
* 功能变更：修复新建会话后 Composer 右侧 ⋯ / 麦克风无法点击或菜单不显示。
* 实现说明：⋯ 菜单 portal 到 body（同 pill 下拉）；WebView2 用 onMouseDown；降低 panel-resizer z-index；禁用态 pointer-events:none。
### 152
* 更新时间：2026-06-20 22:30:00
* 涉及模块：styles.css / Composer.tsx
* 功能变更：思考块与 Composer ⋯ 菜单分割线对齐 Cursor——细实线 `#2b2b2b`，去掉紫色虚线框。
* 实现说明：`.think-card` 改为上下 `border-soft` 实线、透明背景；`.composer-menu-sep` 分隔开关组与操作项；思考计时文字改为 `--fg-dim`。
### 153
* 更新时间：2026-06-20 23:15:00
* 涉及模块：EditorPanel.tsx / SettingsView.tsx / App.tsx / styles.css
* 功能变更：设置改为编辑器 pinned 标签页打开，不再整页覆盖已打开代码；关闭或切换文件标签即可回到代码。
* 实现说明：`DeepSeek 设置` 标签 + 竖向分隔线仿 Cursor；`SettingsView embedded` 隐藏顶栏；打开文件时自动退出设置视图。
### 154
* 更新时间：2026-06-20 23:45:00
* 涉及模块：EditorPanel.tsx / App.tsx
* 功能变更：切换代码标签时设置 pinned 标签保留，仅点 × 才关闭；设置与代码视图分离为 `settingsTabOpen` + `settingsActive`。
* 实现说明：点击文件标签只 `deactivate` 设置；设置内容 hidden 保挂载；再次点设置标签可切回。
### 155
* 更新时间：2026-06-21 00:10:00
* 涉及模块：styles.css / TitleMenuBar
* 功能变更：顶栏加高至 38px、搜索条加宽；搜索栏相对整窗水平居中（仿 Cursor）。
* 实现说明：`.title-menu-bar` 改 flex + `.title-menu-center` 绝对居中；搜索条 28px 高、最大 520px；左右区 flex:1 平衡布局。
### 156
* 更新时间：2026-06-21 00:35:00
* 涉及模块：App.tsx / styles.css
* 功能变更：Chat 新建会话「+」移至右侧固定操作区，多标签时不再被挤掉或覆盖。
* 实现说明：「+」从 `.tabs-scroll` 移到 `.tabs-actions` 首位；标签区仅横向滚动；左侧竖线分隔仿 Cursor。
### 157
* 更新时间：2026-06-21 01:00:00
* 涉及模块：styles.css
* 功能变更：顶栏品牌图标（第二个图标）垂直居中对齐，与侧栏开关一致。
* 实现说明：`.title-menu-left` 改 `align-items: center`；品牌 logo 18px 与 panel-toggle 一致。
### 158
* 更新时间：2026-06-21 01:20:00
* 涉及模块：README.md / .cursor/rules
* 功能变更：README 移除「更新记录」章节，仅保留项目介绍；新增规则 `readme-intro-only.mdc`。
* 实现说明：开发变更继续只写 GUI-CHANGELOG.md；`readme-backup.mdc` 补充内容范围说明。
### 159
* 更新时间：2026-06-21 02:00:00
* 涉及模块：CodeView.tsx / styles.css
* 功能变更：修复代码编辑器无法向下滚动查看长文件的问题。
* 实现说明：去掉固定像素高度，改用 flex + `height:100%`；`.editor-body` 子项绝对叠层；`.cm-scroller` 显式 `overflow:auto`；标签切换/缩放后 `requestMeasure`。
### 160
* 更新时间：2026-06-21 03:00:00
* 涉及模块：guiPrefs.ts / SettingsView.tsx
* 功能变更：思考块英文自动汉化默认开启（未设置过偏好时视为 on）。
* 实现说明：`loadTranslateEnabled` 在 localStorage 无值时返回 true；曾执行 `/translate off` 的用户仍为关闭。
### 161
* 更新时间：2026-06-21 04:30:00
* 涉及模块：translate.rs / useConversation.ts / App.tsx
* 功能变更：修复思考块翻译失败（显示「思考内容翻译失败，已显示原文」）。
* 实现说明：对齐 TUI `apply_reasoning_effort(off)`，翻译请求加 `thinking: disabled`；解析 `content` 并兜底 `reasoning_content`；按原文长度动态 `max_tokens`；翻译不再绑定会话推理模型，改用 `default_text_model`。
### 162
* 更新时间：2026-06-21 05:30:00
* 涉及模块：CodeWhale runtime_threads / engine / ops；Deepseek-GUI App.tsx / types.ts / translation.ts / executeSlashCommand.ts / SettingsView.tsx
* 功能变更：`/translate on` 时在 startTurn 请求注入中文思考指令（system prompt），不再仅依赖后置翻译。
* 实现说明：StartTurnRequest 新增 `translation_enabled`、`locale_tag`；HTTP 路径传给 `Op::SendMessage` 并 `refresh_system_prompt`；GUI 中文界面默认传 `translation_enabled: true` + `locale_tag: zh-Hans`；仍保留英文泄漏时的后置翻译兜底。
### 163
* 更新时间：2026-06-21 06:15:00
* 涉及模块：translate.rs / useConversation.ts / MessageItem.tsx / styles.css
* 功能变更：修复历史思考块仍显示英文且「翻译失败」；长思考块分段翻译。
* 实现说明：翻译固定 `deepseek-v4-flash`；超 2800 字按段落分块；校验译文长度；切换线程/灌入历史后 `reconcileThinkingTranslations` 自动补翻；失败时展示简要错误原因。
### 164
* 更新时间：2026-06-21 07:00:00
* 涉及模块：styles.css
* 功能变更：思考块正文改为正体显示，不再整段斜体。
* 实现说明：`.think-body` 去掉 `font-style: italic`。
### 165
* 更新时间：2026-06-21 07:30:00
* 涉及模块：MessageItem.tsx / styles.css
* 功能变更：思考块标题改为正体；去掉 🧠 图标；折叠箭头改为 Cursor 风格 SVG chevron。
* 实现说明：复用 `TreeChevron`；`.think-label` 与正文均为 `font-style: normal`。
### 166
* 更新时间：2026-06-21 08:00:00
* 涉及模块：useResizablePanels.ts / styles.css / Composer.tsx
* 功能变更：Composer 底栏拉窄时不再隐藏 pill / 发送按钮。
* 实现说明：Chat 最小宽度 380→460px；工具栏 `min-width:400px`、左侧取消隐藏滚动；pill `flex-shrink:0`；模型名不再被 120px 硬截断。
### 167
* 更新时间：2026-06-21 08:30:00
* 涉及模块：useTranscriptAutoScroll.ts
* 功能变更：打开/切换会话时自动滚到最新消息（会话底部）。
* 实现说明：切换 thread 后 800ms 内强制滚底并多次重试，覆盖 REST 历史灌入与 Markdown 布局延迟。
